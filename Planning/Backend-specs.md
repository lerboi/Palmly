...
| **D2** | **Raw image retention** | Spec §9 requires minimal retention; competitors market "we don't store your image" as a trust feature. | **Full-frame camera photos never leave the device** (client crops/normalizes first). The normalized palm/face crop is deleted from Storage **24h after the scan is created** (cron), or immediately on user request — see §9; the "after extraction" wording was corrected 2026-07-17 (D-07), and the `created_at` clock deletes *sooner*, so the marketing claim is if anything understated. Consequence: re-running a *better future extractor* on old scans is impossible unless the user opts into "keep my scan" (settings toggle, default OFF). |
...
- Storage: private buckets, path convention `scans/{user_id}/{scan_id}.jpg`, RLS on `storage.objects` keyed on `storage.foldername(name)[1] = (select auth.uid())::text` (**corrected 2026-07-17 — the audit flagged the `[1]` vs `[2]` mismatch and resolved it against the spec: the SQL is right**). `storage.objects.name` holds the path *inside* the bucket and the bucket is a separate `bucket_id` column, so the segments are `{user_id}/{scan_id}.jpg` and the user id is `[1]`. Verified against live rows: for `name = '<uuid>/<uuid>_feed_4x5.png'`, `foldername(name)[1]` is the user uuid and `[2]` is **NULL** — so the `[2]` version would compare `NULL = auth.uid()::text`, which is NULL, and RLS would deny **every** storage operation. The path convention below writes `scans/...` meaning *bucket `scans`, then the path*, which is what invited the off-by-one; uploads via `createSignedUploadUrl()` so the client never holds broad storage permissions.
...
| Normalized crop (palm/face) | Private bucket | **Deleted 24h after the scan is CREATED** (hourly cron; Storage API deletion — SQL deletes orphan objects). Deleted immediately on scan failure resolution or user request. `image_deleted_at` audited. **Corrected 2026-07-17 (audit C4 / Decision Log D-07): this used to say "24h after successful extraction", which the SQL never implemented — `crops_due_for_deletion` keys the age on `created_at`, deliberately.** Keying on extraction could only ever retain crops *longer* (a crop is spent the moment pass 1 reads it; pass 2 reads features, never the image), which would weaken the D2 promise rather than strengthen it. The spec was wrong; the SQL is right. | Marketing claim we can make honestly: *"Your photos are analyzed, then deleted — usually within a day."* |
