...
- [ ] **R0.T3** 🧑→🤖 Docker deploy of `card-render` (audit A2). Human: start Docker Desktop. Agent: `npx supabase@latest functions deploy card-render --use-docker --project-ref …` (bundles `static_files` wasm+fonts).
...
- [ ] **R0.T5** 🤖 ∥ Emit the ~7 dark analytics events that have live call sites today (audit A7: capture trio, `reading_ready`, `reveal_time_spent`, `invite_clicked` on claim, `notification_pref_changed`); leave purchase/push/install events for their H8/H9/device moments. Update `docs/ANALYTICS.md` in the same commit.
  - Verify: grep shows a call site per event; tsc/lint/jest green.
- [ ] **R0.T6** 🤖 ∥ Hygiene sweep (audit A9, A10): dependency pass (`npx expo install --check` → expo-doctor fully green); one additive migration revoking PostgREST EXECUTE on the trigger-only SECURITY DEFINER functions; update `HowItWorks.md` (cron claim) + the `MVP_Buildplan.md` STATE block (point at Audit-2/Audit-3 reality); commit the Planning reorg sitting in the working tree _(reorg-commit leg discharged early by D0.T0 baseline commit, 2026-07-20)_.
...
- [ ] **R2.T2** 🤖 **Nightly fortunes real**: after R2.T1, confirm tomorrow's 61/61 buckets generate on schedule; re-run today's 4 failed buckets; fortune home shows a real row every morning.
