...
| **Last completed task** | D2.T1 chunk 9-seal — **heritage CHOP-SEAL on rendered cards** (closes F2.T1 §5.4 #2 + F1.T11 fortune-seal). Shared `chopSeal()` = a FILLED claret name-chop across all 3 card classes (was a triplicated outline). Rendered locally + viewed; redeployed card-render `--use-docker` (v10). Deno 218/218. D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — the PROGRESS LOG is authoritative.** Done: chunks 1, 2, 3a, 4a, 4c, 4b, 3b-fortune, 7 (size), 5 (QR=deleted), **9-seal (chop-seal)**. Three card classes render live + budget-guarded + no fakes + heritage chop. Rarity (F2.T8) = honest/no-op (no number, identity in headline); lunar whisper (F1.T11 #4) `[~]` (needs a lunar util, deferred). **REMAINING enhancements:** (a) **chunk 8 — solo/compat label-anchor polish** (fortune dead-band already investigated → acceptable; check solo/compat line-label collisions by rendering LOCALLY: `deno run --config supabase/functions/deno.json --allow-read --allow-write` importing `buildCardSvg`+`renderCardPng` from `file:///…` — no deploy to view, only to ship). (b) **chunk 6 — dark story variant + sheet toggle** (dark PALETTE through all 3 build*CardSvg + a dark variant + client toggle + card-render redeploy — the biggest remaining). (c) **chunk 3b-face** — the `face` SVG class (NEW face motif, mirror `HAND_SILHOUETTE`) + `source_type='face'` path; in-app face-SHARE leg `[~]` behind `FACE_READING_ENABLED` (OFF, F1.T6). **DECISION POINT: the D2.T1 CORE Verify is already met** (all 3 classes live-render 200 PNGs — viewed; <450KB assertion; client previews the real draft PNG — chunk 4a; consent + channel row done; heritage chop). a/b/c are polish — a future run may build them OR judge them non-essential, `[~]`-note the gated ones (dark/face), CLOSE D2.T1, and run **D2.G**. Owed at close: verify a published invite carries a working OG card image (chunk 4a mint→publish — fetch the teaser HTML) + tick the Audit-2 F1.T9/F2.T1/F1.T11 `[~]` legs. **D2.G** = final gate (app tsc/lint/jest + Deno + Node all green; consolidated report; loop ENDS with a PushNotification). **Docker running (28.1.1); card-render v10 live.** |
  ...
  - **PROGRESS LOG (multi-chunk build — this is a ~10-part task, paced across iterations):**
    ...
    - **Chunk 9-seal ✅ (2026-07-20): the heritage CHOP-SEAL on the rendered cards (closes F2.T1 §5.4 #2
      ...
      **218/218**. _Rarity (F2.T8): nothing to build — that leg was already decided (NO fabricated
      ...
      decision, deferred)._
    - **REMAINING chunks (next iterations):** (3b-face) the `face` SVG class — needs a NEW face motif
      in `card-svg.ts` (no face silhouette exists; mirror the `HAND_SILHOUETTE` pattern) + a
      `source_type='face'` card-render path; the in-app face-SHARE leg stays `[~]` behind
      `FACE_READING_ENABLED` (flag-gated OFF, F1.T6). (6) dark story variant + sheet toggle.
      (8) solo/compat label-anchor polish (the fortune dead-band was investigated → acceptable). Then
      **D2.G** (final gate). [face + dark + label-polish are the last enhancements; each can also be
      `[~]`/deferred when closing D2.T1 if judged non-essential to the core Verify.]
