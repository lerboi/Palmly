...
| **Next task** | **D2.T1** 🤖 (share-card craft parity — the deferred Audit-2 F1.T9 tail; card-render live) |
| **Blocked on** | — |
...
| **Last completed task** | D2.T1 chunk 4c — the MARKET-ORDERED channel row (`ShareView`: horizontal `ScrollView`, WhatsApp·LINE·Zalo·IG·TikTok·Copy·More, honest brand monograms — no faked logos) + channel-aware share copy (`composeShareText` → new pure `lib/shareText.ts`, re-exported from `invite.ts`). 4 new jest tests → **jest 68/68**, tsc 0 / lint 0. D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — the PROGRESS LOG is the authoritative remaining list.** Done: chunk 1 (compat SVG), chunk 2 (compat card-render integration + LIVE render, v6), chunk 3a (fortune SVG), **chunk 4a (client "preview == posted" — LIVE signed-URL verify, 74 956-byte PNG)**, **chunk 4c (market-ordered channel row + per-channel text)**. **Recommended next = chunk 4b ("Show my name" consent toggle) — design already settled this run in Decision D3-03:** publish is a byte-COPY of the draft (`_shared/card-publish.ts`) and the solo draft ALWAYS bakes `profile.display_name` (`card-render/index.ts` line 97), so "preview == posted" forces the toggle to pick among PRE-RENDERED drafts (not re-render at publish). BUILD: (1) `card-render` reading branch honors an explicit empty/whitespace `attribution` (renders no byline) and stores it under a distinct variant `feed_4x5_anon` (check `share_cards.variant` is free-text, not an enum, via MCP first); (2) worker-narrative pre-renders BOTH the named `feed_4x5` and the `feed_4x5_anon` draft; (3) redeploy `card-render --use-docker` + redeploy worker-narrative; (4) `ShareView` adds a "Show my name on the card" switch (default ON) that flips `loadDraftCardPreviewUrl(readingId, on ? 'feed_4x5' : 'feed_4x5_anon')` for the preview AND selects which `share_cards` id `ensureInvite`→`publishShareCard` publishes; (5) LIVE-verify via harness (render both variants, sign+fetch each, assert the anon one has no name — assert by rendering/inspecting, since PNG text isn't greppable → compare byte lengths / view). **OR chunk 3b:** `solo face` SVG class (new face motif) + card-render `source_type` integration for `fortune`+`face` (Decision-Log the fortune input/owner: (date,bucket)+user) + live renders. Remaining after: (5) QR encoder (or delete+Decision-Log) → then add the QR tile to the channel row in its market slot (before More); dark story variant; <450KB rasterized-PNG assertion (resvg render test); label-anchor/silhouette polish; card-corner seal (F2.T1); fortune share seal (F1.T11); rarity (F2.T8). **Docker running (28.1.1); card-render v6 live.** |
...
- [~] **D2.T1** 🤖 **Share-card craft parity.** (Gated: only if D0.T3 verified 200.) — IN PROGRESS (multi-chunk; see note). Read the
  ...
  - **PROGRESS LOG (multi-chunk build — this is a ~10-part task, paced across iterations):**
    ...
    - **REMAINING chunks (next iterations):** (4b) "Show my name" consent toggle (gates the byline).
      ⚠️ Design settled this run (Decision **D3-03**): publish is a byte-for-byte COPY of the draft
      (`_shared/card-publish.ts`), and the solo draft ALWAYS bakes `profile.display_name`
      (`card-render/index.ts` reading branch, line 97) — so "preview == posted" forces the toggle to
      pick among PRE-RENDERED drafts, not to re-render at publish. Plan: render a second anonymous
      draft (a `feed_4x5_anon` variant, or `card-render` honoring an explicit empty `attribution`
      for the reading branch) so the sheet can instantly preview + publish either; the toggle selects
      the variant. Needs a `card-render` change + a second pre-render (worker-narrative) + redeploy
      `--use-docker` + live verify. Its own chunk. (3b) `solo face` SVG class + card-render integration
      ...
      toggle. (7) <450KB rasterized-PNG assertion in a resvg render test. (8) label-anchor/
      headline-dead-band polish. (9) card-corner seal (F2.T1), fortune share seal (F1.T11), card-side
      rarity (F2.T8 — honest, no fabricated number).
