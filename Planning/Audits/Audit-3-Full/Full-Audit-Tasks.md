...
| **Last completed task** | D2.T1 chunk 2 — card-render integration + LIVE compat render. `card-render/index.ts` `source_type='compatibility'` branch + `render.ts renderAndStoreCompatCard` (shared `storeCard`); deployed `--use-docker` (v6). Live: seeded pair → 200 + 80 KB compat PNG (viewed: two palms + red thread + "82" ring + names + chips). Deno 213/213. D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — chunk 4 (client sheet honesty) OR chunk 3 (face/fortune classes).** Highest-value next = **chunk 4: `ShareView` "preview == posted"** — the client previews the REAL pre-rendered draft PNG (signed `card-drafts` URL via a new lib helper, since `card-drafts` is private → sign it server-side or via a storage signed-URL call as the owner), a **"Show my name on the card"** consent toggle gating the attribution byline (default per U6 — check spec), and the **market-ordered channel row** (WhatsApp·LINE·Zalo·IG·TikTok·Copy·QR·More) with per-channel pre-composed text firing `invite-create` with the channel tag. RESEARCH `ShareView.tsx` (current CompatPreview/SoloPreview + channel row `onShare`), `lib/invite.ts` (`createInvite` channel), how the client fetches a `card-drafts` signed URL (RLS: share_cards_select_own + storage owner policy). App gates when ShareView touched. OR **chunk 3**: `solo face` (face-shape headline + face motif hero) + `daily fortune` (almanac hero) SVG classes in card-svg.ts + card-render source_type handling + Deno tests + live renders. Remaining after: QR, dark variant, <450KB rasterized assertion, label-anchor polish, seals, rarity (see PROGRESS LOG). **Docker running (28.1.1); card-render v6 live.** |
...
- [~] **D2.T1** 🤖 **Share-card craft parity.** (Gated: only if D0.T3 verified 200.) — IN PROGRESS (multi-chunk; see note). Read the
  ...
  - **PROGRESS LOG (multi-chunk build — this is a ~10-part task, paced across iterations):**
    ...
    - **REMAINING chunks (next iterations):** (3) `solo face` + `daily fortune` SVG classes. (4) client sheet
