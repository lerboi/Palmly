...
| **Last completed task** | D2.T1 chunk 4b — the **"Show my name on the card" consent toggle** (F1.T9, per D3-03). `card-render` `anonymous` flag → byline-less `feed_4x5_anon` draft (no migration); `worker-narrative` pre-renders both; `ShareView` solo switch flips the previewed + published variant. Deployed worker-narrative + `card-render --use-docker` (v7). Deno 217/217, tsc 0 / lint 0 / jest 68/68. LIVE-verified: named (77 000 B, byline) ≠ anon (74 956 B, none). D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — the PROGRESS LOG is the authoritative remaining list.** Done: chunk 1 (compat SVG), chunk 2 (compat card-render integration + LIVE render), chunk 3a (fortune SVG), chunk 4a (client "preview == posted" — LIVE signed-URL), chunk 4c (market-ordered channel row), **chunk 4b (consent toggle — LIVE named≠anon)**. **Recommended next = chunk 3b:** the `solo face` SVG class + card-render `source_type` integration for `fortune` (+ `face`), each live-rendered. RESEARCH FIRST via MCP: a fortune card is keyed on (date, bucket) + a sharing user, NOT a feature_set — so card-render needs a `source_type='fortune'` branch that loads `fortune_templates.content` for the day/bucket and renders `buildFortuneCardSvg` (owner = the sharing user; Decision-Log the input/owner). The `face` class needs a NEW face motif in `card-svg.ts` (there's no face silhouette yet — check `HAND_SILHOUETTE` for the pattern) + a `source_type='face'` path (face feature_sets exist but the FACE door is flag-gated OFF per F1.T6 — so build the card class + render path, mark the in-app face-share leg `[~]` behind `FACE_READING_ENABLED`). Remaining after: (5) real QR encoder (pure-TS, decode-tested) → then add the QR tile to the channel row in its market slot (before More); OR delete + Decision-Log. (6) dark story variant + sheet toggle. (7) <450KB rasterized-PNG assertion — a resvg render test (⚠️ resvg-wasm is deliberately kept OUT of the standing Deno `deno check`/suite per `render.ts` top-comment; a real rasterize test needs its own file + `--allow-read` + the wasm/fonts — scope it so it doesn't destabilize the 217-test gate for every future edge task). (8) label-anchor/headline-dead-band polish. (9) card-corner seal (F2.T1), fortune share seal (F1.T11), rarity (F2.T8 — honest, no fabricated number). **Docker running (28.1.1); card-render v7 live; worker-narrative redeployed.** |
