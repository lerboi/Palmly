...
| **Last completed task** | D2.T1 chunk 3a — the DAILY-FORTUNE card class SVG (`buildFortuneCardSvg` + `deriveFortuneCardContent`; date eyebrow → essence headline → lucky-triad tiles → do-chips → seal; no invented almanac values). Deno 216/216 (was 213). D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — see the PROGRESS LOG for the authoritative remaining list.** Done so far: chunk 1 (compat SVG), chunk 2 (compat card-render integration + LIVE render, v6), chunk 3a (fortune SVG). **Recommended next = chunk 4 (client "preview == posted"):** `ShareView.tsx` shows the REAL pre-rendered draft PNG (a new `lib/invite.ts` or `lib/readings.ts` helper: find the user's `share_cards` row for the reading → `supabase.storage.from('card-drafts').createSignedUrl(storage_path, 3600)` → `<Image>`; falls back to the SVG preview while loading / when absent — works cleanly for SOLO cards where the sharer owns the draft), a **"Show my name on the card"** consent toggle (gates the byline — NOTE: the pre-render bakes `attribution=display_name` in worker-narrative, so toggling OFF needs a re-render without attribution, or the pre-render omits it and publish adds it — Decision-Log the approach), and the **market-ordered channel row** (WhatsApp·LINE·Zalo·IG·TikTok·Copy·QR·More — CHECK available `Icon` names first; brand logos may be absent → labelled buttons) with per-channel text + a channel tag on `invite-create`. App gates (ShareView touched); screenshot best-effort (the real-PNG preview needs an owner with a rendered card — impractical on the anon web export, verify the signed-URL path via a harness instead). **OR chunk 3b:** `solo face` SVG class (needs a face motif — new) + card-render `source_type` integration for `fortune`+`face` (Decision-Log the fortune input/owner: (date,bucket)+user) + live renders. Remaining after: QR encoder (or delete+Decision-Log), dark story variant, <450KB rasterized-PNG assertion (resvg render test), label-anchor/silhouette polish, card-corner seal, rarity. **Docker running (28.1.1); card-render v6 live.** |
