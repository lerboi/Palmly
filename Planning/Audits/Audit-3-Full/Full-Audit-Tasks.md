...
| **Next task** | **D0.T6** (hygiene sweep — deps, RPC-revoke migration, doc truth — A9 + A10) ∥ |
| **Blocked on** | — |
...
| **Last completed task** | D0.T5 — 8 dark analytics events wired at live call sites (`capture_state_dwell`/`_completed`/`_abandoned`, `reading_ready`, `reveal_time_spent`, `invite_clicked{web}`, `notification_pref_changed`, `paywall_page_viewed`); `docs/ANALYTICS.md` updated; gates green; grep confirms ≥1 non-test call site each. |
| **Notes for next run** | D0.T6 (∥, A9+A10): (a) deps — `npx expo install --check` from `app/` until `npx expo-doctor` fully green (audit: 5 pkgs a patch behind). (b) ONE additive migration (new file, expand-contract) revoking PostgREST EXECUTE from `anon`/`authenticated` on the trigger-only SECURITY DEFINER fns: `handle_new_user`, `broadcast_scan_status`, `broadcast_compat_status`, `resolve_awaiting_compat` — LEAVE `is_pair_member`/`thread_owner`/`set_keep_image` (A9); apply via `db push` (the one apply path, D0.T2). (c) docs: fix `HowItWorks.md` "5 scheduled cron drains" claim (reality: A4→D1); update `MVP_Buildplan.md` STATE block to point at Audit-2 + Audit-3 + this ledger. (d) note leaked-password toggle as 🧑 R1.T10 — do NOT attempt (dashboard-only). Verify: `expo-doctor` all pass; `get_advisors(security)` no longer lists the revoked fns; `db push` clean; standing gates + Node suite; `git status` clean. **Docker still running (28.1.1).** |
