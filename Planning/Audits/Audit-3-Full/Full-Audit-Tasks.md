...
| **Last completed task** | D2.T1 chunk 7 — **the <450KB rasterized-PNG budget assertion** (F1.T9). NEW `card-render/render.test.ts` runs all card classes/variants through the real `renderCardPng` (resvg) → valid PNG + < 450 000 B. Standing Deno suite **218/218**; no deploy. D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — the PROGRESS LOG is the authoritative remaining list.** Done: chunks 1, 2, 3a, 4a, 4c, 4b, 3b-fortune, **7 (size assertion)**. All three card CLASSES render live (solo/compat/fortune) + are PNG-budget-guarded. **Recommended next = pick a bounded chunk:** (a) **chunk 5 — the QR decision (HONESTY-relevant):** the story-variant "scan to compare" block (`card-svg.ts` ~line 180) is a FAKE — 3 black squares + text, NOT scannable. Per "never fake": either build a real pure-TS QR encoder (decode-tested) OR delete the placeholder + Decision-Log. ⚠️ Pre-render timing: the card is rendered BEFORE an invite is minted, so a per-invite QR can't be baked — a real QR could only encode a GENERIC palmly.app URL (still useful for story posts w/o tappable links). Weigh a full encoder (~400 LOC, big) vs. delete. OR (b) **chunk 6 — dark story variant + sheet toggle** (thread a dark PALETTE through all 3 build*CardSvg + a `feed_4x5_dark`/`story_9x16_dark`-style variant + client toggle + card-render redeploy — bigger than it sounds). OR (c) **chunk 3b-face** — the `face` SVG class: a NEW face motif in `card-svg.ts` (no face silhouette exists — mirror `HAND_SILHOUETTE`) + a `source_type='face'` card-render path; the in-app face-SHARE leg stays `[~]` behind `FACE_READING_ENABLED` (flag-gated OFF, F1.T6). Remaining beyond those: (8) label-anchor/headline-dead-band polish (the fortune card shows a visible headline→triad dead-band; solo/compat label anchors too). (9) card-corner seal (F2.T1), fortune share seal (F1.T11), rarity (F2.T8 — honest, no fabricated number). Then D2.G (final gate). **Docker running (28.1.1); card-render v8 live.** |
...
- [~] **D2.T1** 🤖 **Share-card craft parity.** (Gated: only if D0.T3 verified 200.) — IN PROGRESS (multi-chunk; see note). Read the
  ...
  - **PROGRESS LOG (multi-chunk build — this is a ~10-part task, paced across iterations):**
    ...
    - **REMAINING chunks (next iterations):** (3b-face) the `face` SVG class — needs a NEW face motif
      ...
      `FACE_READING_ENABLED` (flag-gated OFF, F1.T6). (5) real QR encoder (pure-TS, decode-tested) → then
