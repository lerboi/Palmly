...
| **Last completed task** | D2.T1 chunk 5 — **the QR decision = DELETE + Decision-Log** (F1.T9, D3-06). The story-variant "scan to compare" block was a non-scannable FAKE; removed it + updated the story test. A real QR needs the per-share invite URL (minted after pre-render → share-time, deferred). Redeployed card-render `--use-docker` (v9) to retire the fake. Deno 218/218. D2.T1 stays `[~]`. |
| **Notes for next run** | **CONTINUE D2.T1 — the PROGRESS LOG is the authoritative remaining list.** Done: chunks 1, 2, 3a, 4a, 4c, 4b, 3b-fortune, 7 (size assertion), **5 (QR = deleted, D3-06)**. Three card classes render live (solo/compat/fortune), PNG-budget-guarded, no fakes. **REMAINING (pick one; all are bounded card-svg/polish work — no more big backend integration):** (a) **chunk 8 — label-anchor polish.** ⚠️ INVESTIGATED the fortune "dead-band" this run: the triad tiles (`tileY=700`) are already ≈ vertically centered between the headline bottom (~420) and the seal rail — computing the balanced center gives ~668, so the airiness is inherent to sparse content (3 lines + 3 tiles + 3 chips in a 1350px card) and reads as intentional premium whitespace, NOT a lopsided defect. No honest fortune Y-tweak to make. Chunk 8's real value is the SOLO/COMPAT **label anchors** (line labels colliding with strokes/silhouette — Audit-2 F2) — verify by rendering LOCALLY via a Deno script that imports `renderCardPng` (no deploy to view, only to ship). (b) **chunk 9 — the seals/rarity trio**: card-corner seal (F2.T1), fortune share seal (F1.T11 — a lunar-date whisper), card-side rarity (F2.T8 — HONEST framing, NO fabricated number). (c) **chunk 6 — dark story variant + sheet toggle** (thread a dark PALETTE through all 3 build*CardSvg + a dark variant + client toggle + card-render redeploy — the biggest remaining). (d) **chunk 3b-face** — the `face` SVG class (NEW face motif, mirror `HAND_SILHOUETTE`) + a `source_type='face'` card-render path; in-app face-SHARE leg `[~]` behind `FACE_READING_ENABLED` (OFF, F1.T6). **After all chunks → D2.G** (final gate: app tsc/lint/jest + Deno + Node all green; consolidated report; then the loop ENDS with a PushNotification). **Docker running (28.1.1); card-render v9 live.** |
...
- [~] **D2.T1** 🤖 **Share-card craft parity.** (Gated: only if D0.T3 verified 200.) — IN PROGRESS (multi-chunk; see note). Read the
  ...
  - **PROGRESS LOG (multi-chunk build — this is a ~10-part task, paced across iterations):**
    ...
    - **REMAINING chunks (next iterations):** (3b-face) the `face` SVG class — needs a NEW face motif
      ...
      (8) label-anchor/headline-dead-band polish. (9) card-corner seal (F2.T1), fortune share seal
      (F1.T11), card-side rarity (F2.T8 — honest, no fabricated number).
