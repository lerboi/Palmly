...
| Field | Value |
|---|---|
| Current phase | U6 — share / chat / history / paywall / account |
| Next task | U6.T3 |
| Blocked on | — |
| Waiting on human | — |
| Last run | 2026-07-25 |
| Last completed task | U6.T2 |
...
| D65 | 2026-07-26 | **A fabricated compat card is now UNREPRESENTABLE, not merely avoided.** Four always-present props (`score`, `partnerName`, `blurb`, `chips`) let the route fill in `score ?? 0` / `'Your match'` and the sheet supply invented prose as DEFAULTS. They collapse into one optional `pair` — absent means invite-first, so there is no combination of props that renders a card without a pair. `hasRealPair` additionally rejects a zero score and the `'Your match'` placeholder, because that is exactly the shape the old path produced. |
| D66 | 2026-07-26 | **The card's blurb and chips now come from the pair's OWN narrative and sub-scores.** `loadCompatShare` returned only score + name, so the blurb and the four chips were `ShareView` defaults — meaning a real user's real card carried invented copy about a real person. It now selects `narrative, sub_scores` (an existing RLS-scoped read, no schema change) and maps them through the same `toPairData` the pair screen uses; an empty narrative renders nothing rather than a default. |
| D63 | 2026-07-26 | **The bottom "Share" button now reports the `more` channel — the only CHANNELS key that describes what it does.** U6.T1's verify demands `grep onShare('share')` → 0, and `'share'` was a channel value that existed nowhere in `CHANNELS`, so the analytics had a category no dashboard could join. The big button and the More tile open the SAME generic OS sheet, so one honest value beats two names for one action. |
| D64 | 2026-07-26 | **Reverted an accidental `prettier --write` and re-applied the edits by hand.** Formatting the file to the repo's own config still rewrote ~150 lines I had not touched (import wrapping, mostly), turning a 138-line change into a 403-line one and burying the task in churn. The loop's rule is scoped commits; a formatter run is its own change, not part of CO-12. |
| D60 | 2026-07-26 | **The auto-presented compat share modal is DELETED, not delayed — the owner-taste call U5.T3 asked for.** It fired 2s after `complete`, i.e. **on top of the still-running choreography**, taking over the screen at the exact moment the user was reading their score. The alternative in the direction ("a Share this pill instead of a modal takeover") already existed as the in-page "Share this match" CTA, so the modal added nothing but an ambush. `AUTO_PRESENT_DELAY_MS` / `shouldAutoPresent` / the per-pair seen-store are now unused by the route (left in `lib/` for U6.T2, which owns the share side). |
| D61 | 2026-07-26 | **The sub-score label is `flex: 1`, not shrink-to-content — measured, after the first fix failed.** `width: 72` clipped at 1.3× (CO-16), so I first made the label content-sized. At 1.3× that measured **five different bar-track left edges (118–159px) and five different widths** — no clipping, but the bars no longer lined up, which is the same defect wearing different clothes. `flex: 1` on the label and `flex: 1` on the track splits every row's leftover identically: re-measured at 1.3×, **one** track left (59), **one** width (127), **one** value right (357) across all five rows. |
| D62 | 2026-07-26 | **`toPairData` + the sub-score label/order maps moved from `lib/compat.ts` to `lib/compatCopy.ts`.** `compat.ts` imports AsyncStorage and supabase, so a test that touches it dies in jest — which is why the mapping had no coverage and why `sub_scores` keys could drift from the icon map unnoticed. `compatCopy.ts` exists precisely for the pure half; `compat.ts` re-exports `toPairData` so no caller had to change. |
| D57 | 2026-07-26 | **`ConfirmSheet` became a shared `components/ui` primitive rather than a third hand-rolled `Modal`.** `BirthDateSheet` already hand-rolls scrim + handle + slide-up; SN-7 needed the same shape again, and U6 will need it once more. The scrim resolves to **cancel**, never confirm, so a stray tap on the dim area can't be the destructive branch. |
| D58 | 2026-07-26 | **The failure screen now shows ONE CTA — the two-CTA divergence is blocked by this ledger's own scope, and is recorded rather than faked.** CO-13's defect is two buttons `replace`ing to the SAME route, so the choice was theatre. Direction §4.5 wants "Try again → camera" and "Upload a photo → primer in upload mode". Neither half is reachable from here: a direct `/palm` link would be **the app's first path to the camera that skips the versioned biometric consent** (`recordCameraConsent` is called only by `(capture)/primer`), and an upload-mode primer requires editing `(capture)/*`, which is OUT OF SCOPE (R3 paused). One honest CTA to `/primer` — the door that itself offers both camera and upload — removes the defect without inventing a consent hole. **Pending leg: restore the two diverging CTAs once `(capture)/primer` gains an upload-mode entry.** |
| D59 | 2026-07-26 | **The notify holding state lives INSIDE `AnalyzingView`, not on a new route.** SN-7's complaint is that "Notify me" `replace`d to `/` — the marketing launcher. A dedicated `/waiting` route would need its own scan subscription and its own back-stack story; a state inside the loader keeps the scan polling while the user lingers, and its one CTA goes to Today. `onNotifyMe` no longer navigates at all — it asks for the permission and returns, which also makes the handler honest about what it does. |
| D54 | 2026-07-25 | **The ring's creep is asymptotic, not linear, and it stops at 92% — never 100%.** CO-13 asks for 75→92% during extraction. A linear ramp would need a guessed duration and would either arrive early (and then park again, the same bug) or crawl. `CREEP_FROM + (CREEP_TO − CREEP_FROM)·(1 − e^(−t/20s))` always moves, never arrives, and never claims completion the pipeline hasn't reached — the narrative stage is what closes the ring. Pinned by a test that walks 2s steps across a 120s window and asserts strict monotonicity. |
| D55 | 2026-07-25 | **The photo is positioned from the ring's own centre, and the ring owns it.** It used to be a sibling with a hand-tuned `top: 18` inside a 288px box — 28px above the axis (CO-13). Rather than fix the number, the crop moved INSIDE `ProgressRing`, which computes `left/top = c − photo/2` from the same geometry it draws the circles with, so the two cannot drift apart again. Measured in the DOM: dx 0px, dy 0px. |
| D56 | 2026-07-25 | **A first scan traces an ABSTRACT motif; a rescan traces the reader's real lines.** SH-7's fix is not just an a11y label — the stage copy said "Tracing **your** heart line…" over a fixture. The route now loads the newest stored reading's geometry (existing RLS-scoped client reads) and passes `ownGeometry`; with it, both the possessive copy and "Your palm line diagram" are true. Without it (a first-timer, or while the read resolves) the copy is "Tracing the heart line…" and the label is "Palm illustration". No backend work: this scan's own geometry genuinely does not exist until extraction ends. |
| D53 | 2026-07-25 | **The litmus needs a saturation floor, and the gate shot needs to be the WHOLE page.** Two method fixes, both found by running it: (1) the wide accent filter (r−g > 35) counts Chrome's **subpixel text antialiasing** — tan fringes on glyph edges — as accent, which produced four "unexplained" sub-1% bands. `elementFromPoint` named them: the pending privacy badge and the error body copy, i.e. plain text. Every real accent pixel measured here clears r−g ≥ 97 (CTA `#d13b27`, claret seal `#9e3c2f`, dark accent `#e47a68`), so `STRICT=1` raises the floor to 70 and the phantom bands vanish. (2) A viewport-height shot only litmuses the first screen; the reveal is 2488pt tall. Gate shots now include a full-page 390×2500 capture. **Both apply to U8.G.** |
| D51 | 2026-07-25 | **SH-8 was fixed at the PRIMITIVE, not at the two call sites the finding names.** `PrivacyBadge`'s own default was the lie ("Photo deleted"), so every present and future default usage inherited it — the audit found two, and there were four. The default is now the canonical promise from `trustCopy.ts`, and "deleted" is reachable only by passing `deletedLabel(...)`, which needs a real `image_deleted_at`. Consequence I had to fix in the same task: the honest string is a full sentence, and title + gear + sentence on one 390pt row truncated "Your readings" to "Your rea…" — the shelf's badge moved to its own line under the header (still one signal per surface). |
| D52 | 2026-07-25 | **SH-16 is fixed by construction, not by adding `setGeometry` to the reset list.** The route held six `useState`s and the reset block remembered five; adding the sixth would have left the same trap for the seventh. The per-reading state is now ONE object with pure `initialRevealLoad()` / `loadedRevealLoad()` in `reveal.ts`, so the reset replaces the whole thing and *cannot* forget a field — and, being pure, it is unit-tested (every field must change on load, which is what makes a missed reset detectable). `deletedLabel` moved to the same pure module for the same reason: it was untestable inside a `.tsx` full of RN imports. |
| D47 | 2026-07-25 | **The `silhouette` prop was REMOVED, not respected.** The ledger offered either. `showSilhouette = isMini || silhouette` meant every one of the five `silhouette={false}` call sites (all ≤96px) got a silhouette anyway, so honoring the prop would have *changed five screens' rendering* to something no one has ever seen, while U6.T4's own line ("silhouette embraced") says the intent for those thumbs is silhouette ON. Deleting the prop keeps every pixel identical and removes the lie from the source. Two comments describing "silhouette off" thumbs were corrected with it. |
| D48 | 2026-07-25 | **The eight face-section icons are approximations from the existing set, not new glyphs.** The set has no eye/brow/nose/mouth/ear icon, and Direction §4.4 #2 names *existing* icons (`face`, `mind`, `heart`, …) for exactly this job, so no icons were added. Each is tied to that section's own tradition line — eyes are the feature physiognomy weighs most (`sparkle`), the nose is the face's "mountain", read for drive (`compass`), the mouth reads for warmth (`heart`), the under-eye for vitality (`life`). `ears` → `globe` is the weakest of the eight and is the first to revisit if a facial-feature icon set is ever drawn. |
| D49 | 2026-07-25 | **`CompareCard`'s centring was fixed here, not left for the gate.** U4.T1 flattened its elevation and wrote "Flat and left-aligned" in the comment — but the title, body and icon were still centred, so the comment described a layout that did not exist and Direction §4.4 #3's "left-aligned" was half-done. It now shares the section cards' marker-left/text-right shape. Fixed in U4.T3 because this task owns reveal card composition and U4.G would only have re-found it. |
| D50 | 2026-07-25 | **"Elevation unified (hero block only `md`)" was already true on arrival — recorded rather than re-done.** After U4.T1 flattened `CompareCard`, `grep elevation` over `RevealView.tsx` returns only `sm` (section, locked, survey) and the reveal has no hero *card* at all (the hero is a bare diagram + text). So the CO-4 defect — one `md` island among `sm` siblings — is gone, and nothing on the page is raised above `sm`. |
| D46 | 2026-07-25 | **The share pill's icon and label are INK, not accent.** Direction §4.4 prescribes the surface/border/shadow treatment but is silent on the label color, and an accent label would be *permitted* (interactive is a sanctioned use). Ink is still the right call: §1 P1 wants accent scarce, and the pill replaces a bright claret seal precisely because a loud floating mark read as decoration. Affordance now comes from the fill + border + shadow + the actual word "Share". The pill also KEEPS the earned-scroll gate (240px) — §4.4 says "appearing after the hero as now". |
| D44 | 2026-07-25 | **`size="md"` alone did NOT fix the survey overflow — the row also had to wrap.** My first measurement asserted the buttons fit within the 320pt VIEWPORT and passed. Re-measuring against the CARD's content box (x33→r287) showed "Not sure" running to 320 — a 33pt overflow. The lenient assertion nearly banked a false green. Fixed with `flexWrap`, the same treatment the Lucky row got in U3.T2, and re-verified: "Not sure" now wraps to x49 on a second line. |
| D45 | 2026-07-25 | **"Already added" is answered from the reader's own shelf via `loadHistory()`**, in an effect keyed on the resolved `kind` — the other-kind question cannot be answered until we know which kind THIS reading is. The shelf does not record hand-side per row, so "has the other hand" is approximated as *more than one palm reading*. Recorded because it is an approximation, not a fact: a user with two right-hand scans would stop being offered the left. Exact side needs a `feature_sets.side` read, which is backend-shaped and out of scope. |
| D42 | 2026-07-25 | **U3.T3 wired the chat `autoFocus`, because it landed FIRST.** The task says "whichever lands second wires the focus" and names U6.T4; chat is actually U6.T3, and neither had landed. `ChatThread`/`InputBar` now take `autoFocus`, set only when a prefill exists — arriving with a question already typed should put the keyboard up. **U6.T3 will find this done**; its remaining chat scope is untouched. |
| D43 | 2026-07-25 | **The lock line is ink, not `premiumInk`.** Direction §4.2 asks for a `lock` icon + `small/secondary`; the old line was low-contrast gold, which CP-6 calls out. Gold on this line would also put a third hue back on a card U3.T1 just reduced to ink + two semantic marks. The champagne stays on the paywall, where it marks the actual product. |
| D41 | 2026-07-25 | **The Lucky row's breakpoint is a pure `luckyColumns(width)` rule, not an inline ternary.** This repo has no component renderer, so a rule buried in JSX would be untestable; as a function it gets unit-tested AND the rendered result is measured separately by DOM probe. Thirds use a **28%** basis, not 33%: three cells at exactly a third plus gaps overflow again, which is a subtler replay of the original bug. |
| D39 | 2026-07-25 | **Added `/dev/fortune-premium`, closing the gap D16 flagged.** The premium almanac card — Do/Avoid rows and the Lucky row, i.e. the exact surface CO-6 calls out for overflow — had NO dev route, so it could not be seen device-free at all. It is the third such gap this ledger has hit (premium card, `(reading)/pair`, the loading/error states); the pair one is still owed by U5.T3. |
| D40 | 2026-07-25 | **Fixed "Colour" → "Color" here rather than deferring to U7.T1.** US spelling is a STANDING rule in this ledger, not U7-scoped work, and the string sits in the component U3.T1 recomposes — leaving a known-wrong word visible in the shot I was taking would have been worse than the tiny scope stretch. U7.T1's sweep will simply find nothing left at this site. |
...
- [x] **U3.T1** 🤖 Recompose `FortuneCard` (CC-2, CO-6): eyebrow sans icon-chip; essence in the
...
- [x] **U3.T2** 🤖 Lucky row responsive (CO-6): compass icon rotated per direction (map with a
...
- [x] **U3.T3** 🤖 Free state + bridge (CP-6, CO-14-part): single lock line per Direction §5;
...
- [x] **U4.T1** 🤖 Tail order + consolidation (CO-2, SH-10): reorder to sections → Compare
...
- [x] **U4.T2** 🤖 Share affordance + Done (CO-3): replace SealFab with the labeled share pill
  ...
  - Verify: standing gates; reveal shot shows one stamp + the labeled pill within safe area;
    ...
    view.
- [x] **U4.T3** 🤖 Section visuals (CO-5): distinct thumbs — 4 line-sections keep mini palms
...
- [x] **U4.T4** 🤖 Reveal honesty (SH-8, SH-16): pending badge → "Photo deletes within 24
...
- [x] **U4.G** 🚦 Reveal gate: standing gates; shots light/dark/320 of ready+pending+error to
  `docs/checkpoints/audit4/u4-gate/`; accent litmus; update STATE.

## U5 — Analyzing + Pair (Direction §4.5, §4.6)

- [x] **U5.T1** 🤖 Analyzing geometry & honesty (CO-13, SH-7): pad the ring SVG viewport (glow
...
- [x] **U5.T3** 🤖 Pair mechanics (CO-16, SH-15, CC-4). ⚠ **Add a `/dev/pair-*` fixture route first** — `(reading)/pair` is currently unreachable device-free, which left U1.T2's pair leg `[~]` (D24). Then: real geometries when stored (fallback
...
- [x] **U5.G** 🚦 standing gates; shots to `docs/checkpoints/audit4/u5-gate/`; update STATE.

## U6 — Share, Chat, History, Paywall, Account (Direction §4.7–§4.11)

- [x] **U6.T1** 🤖 Share sheet layout (CO-12). ⚠ **The `scrollable` leg ALREADY LANDED on 2026-07-25** (owner reported the sheet looked "distorted / overlapped" while U0 was in flight — see Build Log + D14). Do not re-do it; the remaining scope is everything else in this line:  edge-to-edge channel row with
  ...
  - Verify: standing gates; 320×568 + 375 shots — nothing overlapped/below fold; grep
    `onShare('share')` → 0.
- [x] **U6.T2** 🤖 Share compat honesty (SH-7, CO-12): no score-0/"Your match" path — from
...
- [ ] **U7.T1** 🤖 Copy sweep. ⚠ **U4.G measured the `&apos;` sweep at 11 sites** (`grep -rn "&apos;" src --include=*.tsx`), incl. the reveal's error copy "We couldn&apos;t load your reading" — visible in the u4-gate shots. The `→` arrow that U0.T5's errata scoped away was fixed at U4.G (only rendered instance).: implement every Direction §5 rewrite; single canonical legal +
...
|---|---|---|
| 2026-07-26 | U6.T2 | **The share sheet can no longer offer a match that doesn't exist (SH-7).** Opening the compat tab from a reveal passes no `pairId`, so the sheet rendered a finished-looking card reading **0 out of 100** for **"Your match"** — under an invented blurb — and then invited the user to send it to a friend. The compat tab is now **invite-first** when there is no pair: red thread, "Compare palms with a friend", and the channel row that sends the invite — **no score ring, no name, no prose**, shot at `/dev/share-compat-invite`. The four compat props collapsed into one optional `pair`, so the fabricated state is not representable (D65), and `hasRealPair` rejects a 0 score and the `'Your match'` placeholder specifically. **The card's blurb and chips are now the pair's own** narrative headline and dimension labels, read through the same `toPairData` the pair screen uses (D66) — they were `ShareView` defaults, so a real card about a real person carried invented copy. Verify grep: the default blurb string appears **0** times outside `/dev`. The partner palm keeps `differentiateGeometry` as its documented fallback (no partner geometry exists client-side). 2 new tests. Gates green, 153 tests. |
| 2026-07-26 | U6.T1 | **The share sheet's channel row stops pretending it ends at the gutter (CO-12).** The row sat inside the screen's 16pt padding, so the last tile was **clipped flat at the padding edge** and the row read as complete. It is now edge-to-edge with the gutter as content padding plus a trailing fade — **measured at 390: row spans x0→390 (was inset 16/16), first tile still at x16, 214px of overflow, and the TikTok tile literally straddles the viewport edge**, which is the peek CO-12 asks for. **One preview frame for PNG and vector:** the server-rendered card was capped at 340pt while the vector previews ran full-bleed, so the same card changed width depending on which one happened to load — the compat preview now measures **exactly 340, centred (x25→365)**. "Copied" no longer outlives the link it refers to: a framing change, a name toggle or a tab switch invalidates the mint, and all three now clear the confirmation with it. `grep onShare('share')` → **0**: the bottom Share button reports the `more` channel, the only CHANNELS key that matches what it does (D63). Shot at 375 and 320 — nothing overlapped. Legs already closed by earlier tasks and cited rather than redone: `editorialTitle` (U0.T6), the toggle's visible off-state via `trackOff` (U0.T2), the copied icon+label (U0.T5), the scroll itself (D14). Gates green, 151 tests. |
| 2026-07-26 | **U5.G** 🚦 | **Analyzing + pair gate PASSED.** 14 shots — analyzing start/mid/overrun/failed and pair ready/partial/waiting across light, dark and 320 — to `docs/checkpoints/audit4/u5-gate/`. **Accent litmus, every band named:** the analyzing states are **100% ONE band — the progress ring**, which §4.5 explicitly grants as the screen's single accent moment (overrun adds its "Notify me" tonal label at 16%); failure is **98.2% the "Try again" CTA** plus a 1.8% `#aa3529` mark that is the `warning` glyph in **danger crimson, not vermilion** (the three-reds discipline holding); pair-ready is 94.7% "See my full reading" + 2.9% "Share this match" + 0.9% "Done" — three interactive — plus a 1.5% `#9f3e31` **claret red thread**, the motif's sanctioned home (D7); pair-waiting is its "Remind Mira" CTA plus the same thread. **Non-interactive VERMILION on every screen in this phase: 0** (limit 2). Identical band structure in dark. **320pt: 0 non-page pixels in the outer 2pt margins** on all three 320 shots, and the overrun scroll was measured in U5.T2 (760pt of content in a 568pt viewport, CTA reachable). Standing gates: typecheck 0, lint 0, 151 tests. |
| 2026-07-26 | U5.T3 | **The pair reveal is verifiable device-free for the first time, and its mechanics are fixed (CO-16, SH-15, CC-4).** Three `/dev/pair-*` routes finally close the gap D24 flagged in U1.T2 — `(reading)/pair` needs a live pairId + session, so this screen went unshot through three ledgers. **CO-16, each measured:** `DIM_ICON` was keyed on **display copy** ("Emotion", "Energy"), so a label rewrite would have silently greyed every dimension icon to `sparkle` — it now keys on the stable server key, and a test asserts every key `toPairData` can emit has an icon. `SubScoreBar`'s fixed `width: 72` label + `width: 28` value broke at the app's own 1.3× Dynamic Type cap; at 1.3× the rows now measure **one** track left (59), **one** width (127) and **one** value right (357) across all five — bars aligned, zero overflow, zero clipping (D61 — my first fix passed "no clipping" while leaving five different bar starts). An empty narrative section printed a **bare heading over nothing**; blocks with no prose no longer render. The ScoreRing's a11y sentence sat on a non-`accessible` View, so a screen reader read the numeral instead — it is one `accessible` node now. The success haptic re-fired on **every remount**; it is claimed once per pairId (3 tests). The share modal that **auto-presented over the running choreography** is deleted in favor of the in-page CTA (D60). **SH-15:** the 402 gate ran AFTER render, so a gated user saw this screen and was then yanked to the paywall — nothing renders until the gate answers; and the waiting state, which said "Weaving your red thread…" forever with no timeout or action, now escalates calm → slow → **"Remind Mira"** at 45s. **SH-7 (part):** the caller's own palm is their real stored geometry when they have one, instead of a fixture on both sides; the partner's stays `differentiateGeometry`, the documented fallback, since no partner geometry exists client-side. CC-4's gold numeral was already `premiumInk` (D6). `toPairData` moved to the pure module so it could be tested at all (D62). 9 new tests. **Standing device legs unchanged:** the physical haptic and a real TalkBack pass. Gates green, 151 tests. |
| 2026-07-26 | U5.T2 [~] | **The analyzing screen stops being a trap (SN-7, CP-4, CO-13).** Back abandoned an in-flight scan **silently** — and `/analyzing` cannot be re-entered, so the reading was simply gone; it now opens a leave-confirm sheet (new shared `ConfirmSheet`, D57). "Notify me" `replace`d to `/`, **the marketing launcher**, with no route back to anything; it now lands a holding state inside the loader — "We'll ping you when it's ready … waiting in Readings" + **Back to Today** (D59). **All three proven with TRUSTED CDP clicks, not by rendering alone:** Back → sheet reads "Your reading is still brewing / Leave anyway / Keep waiting", and "Keep waiting" returns the page to the stage copy; Notify → the holding screen with its CTA present. **The 320×568 overrun overflow is fixed and measured:** content is 760pt in a 568pt viewport, and the notify CTA's bottom sat at **662 — 94pt below the fold, unreachable**; the screen now scrolls and the CTA measures fully visible (y450–470) after scrolling. CP-4: `FAILURE_DEFAULT` blamed the reader's lighting for every unclassified failure including our own server faults — it is now "That one didn't take — let's try again.", and since the pipeline emits no lighting reason, lighting advice appears nowhere. `[~]` PENDING: Direction §4.5's **two diverging failure CTAs** are not built — "camera" would skip the versioned biometric consent and "upload mode" needs an edit to `(capture)/primer`, which is out of scope (D58). One honest CTA ships instead of two identical ones. Gates green, 142 tests. |
| 2026-07-25 | U5.T1 | **The analyzing ring stops parking, stops clipping, and stops calling a fixture "your palm" (CO-13, SH-7).** The ring drew at `r = size/2 − 6` with a 14-wide glow, putting the halo **1px outside its own viewport** — sliced flat on all four sides; `ringGeometry()` now pads the viewport and the DOM measures the glow's outer edge at 148.5 against a 151 half-viewport (**2.5px clearance, no clip**). The captured crop sat at a hand-tuned `top: 18` in a 288px box — **28px above the axis**; it moved inside `ProgressRing`, which positions it from the same centre it draws the circles with (D55), measured **dx 0px, dy 0px**. Progress was `(stage+1)/4` and `stageFor` caps at stage 2, so the ring **parked at 75%** through the slowest stretch of the pipeline; `analyzingProgress()` creeps asymptotically toward 92% (D54) — the DOM reads dash-offset 150.03/889.07 = **83.1% at 20s**, still moving. The 0% round nub is gone by construction (no arc element below 0). **SH-7:** the loader told every first-time user "Tracing **your** heart line…" over `PREVIEW_GEOMETRY` labelled "Your palm line diagram" — three false claims about a fixture. Copy is now non-possessive and the label is "Palm illustration" unless the route finds a stored reading, in which case it traces the reader's REAL lines and earns the possessive (D56). §5 copy: "Trouble reaching the server — still trying…" → "Still connecting…". Step dots were already ink (U0.T3). 7 new tests; 4 shots incl. 320×568. **Not fixed here and still owed by U5.T2:** at 320×568 the overrun state overflows the screen — the ring is cut off at the top and the notify CTA runs past the bottom, exactly as CO-13 describes; that task owns the scroll. Gates green, 142 tests. |
| 2026-07-25 | **U4.G** 🚦 | **Reveal gate PASSED.** 13 shots — ready/pending/error × light/dark/320, face × light/dark, plus a **full-page 390×2500** ready capture in both schemes — to `docs/checkpoints/audit4/u4-gate/`. **Accent litmus by band analysis (D21), every band named from evidence rather than guessed** (`elementFromPoint` on the raw coordinates, plus each band's mean color): on the whole 2488pt reveal the accent is 86.5% ONE band — the h52 "Compare palms" CTA at `#d13c28` — then the claret `#9e3c2f` ReadyStamp (6.1%), the hero palm's lit lines (3.7%), the "How Palmly reads" link (1.8%), the "Done" header button (1.0%), the CompareCard's claret thread glyph (0.3%) and two section-thumb lit lines (0.5%). **Non-interactive VERMILION occurrences: 0** (limit 2); the two non-interactive reds are both claret and both sanctioned by name — the arrival stamp (§4.4 #1) and the thread motif (D7). Identical band structure in dark. Pending is **100% one band** (the drawing palm's signature lines); error is 94.5% "Try again" + 5.5% "Go back", i.e. two buttons and nothing else. **320pt: 0 non-page pixels in the outer 2pt margins** on all three states. Method fixes recorded for U8.G (D53): a saturation floor, because subpixel text AA was being counted as accent, and a full-page shot, because a viewport shot only litmuses the first screen. One defect found and fixed while gating: the TrustFooter still rendered a typed **"→"** — the exact glyph class CO-8 retired, which survived because U0.T5's errata scoped the grep away from `→`; it is now a `chevron` icon. Standing gates: typecheck 0, lint 0, 135 tests. |
| 2026-07-25 | U4.T4 | **The privacy badge stops claiming a deletion the app hasn't checked (SH-8).** `PrivacyBadge` DEFAULTED to "Photo deleted" — rendered by the pending reveal (over a photo that is by definition still being processed), the history header, and the claim screen. The default is now the canonical `Photo deletes within 24 hours` from `trustCopy.ts`; "deleted" is reachable only through `deletedLabel()`, which needs a real `image_deleted_at` (D51). `deletedLabel` also stopped hand-rolling a 12-hour AM/PM clock — wrong copy in every 24-hour locale — and now carries the DATE when the deletion wasn't today, so a three-week-old reading no longer says its photo went "at 4:15 PM" as if that were this afternoon. **SH-16 fixed by construction (D52):** the route's six `useState`s became one object with pure `initialRevealLoad()`/`loadedRevealLoad()`, so opening reading B can no longer draw reading A's palm — the reset replaces the whole state instead of listing five of six fields. Verify grep: all four default `<PrivacyBadge />` usages now render the promise; the single "deleted" call site is data-backed. **10 new tests** (today/older/kept/absent/invalid/locale; initial-state shape; every-field-resets). Shot the history header + pending reveal — and caught a regression the honest string caused, the longer sentence truncating "Your readings" to "Your rea…", fixed by moving the badge below the header. Gates green, 135 tests. |
| 2026-07-25 | U4.T3 | **Half the reveal's thumbnails stop being the same picture (CO-5).** `SECTION_LINE` mapped only the four line sections, so hand shape, mounts and markings each fell through to a mini palm with *nothing highlighted* — identical props, therefore byte-identical renders: three interchangeable grey palms on one page. They now take feature icons (`palm` / `elements` / `sparkle`), and all EIGHT face keys are mapped instead of two (eyes, brows, nose, mouth, ears and canthus had all collapsed onto the same `face` fallback — D48). **The mini highlight was retuned by fixing a real ratio bug:** minis doubled the ink (F0.11) but left the underlay at hero width, so the halo-to-ink ratio collapsed from 2.86× to 1.43× — the glow stopped surrounding the line and became a slightly fatter copy of it. `diagramWeights()` (pure, 6 new tests) now scales both layers, holding the ratio size-invariant, and rests the mini glow brighter. **Measured before/after on the same probe, same fixture:** accent coverage per lit thumb 0.50→1.15%, 0.41→0.98%, 0.33→0.74%, 0.46→1.05% — the lit line more than doubled its presence at 64px. **Distinctness measured in pixels, not by eye:** each of the 8 thumbs on the new `/dev/reveal-sections` route was scrolled into view and captured as its own clip — 8/8 distinct hashes, 8/8 distinct markup. Locked titles now measure 18px/NotoSans_700Bold, **identical to a free section title** (CO-4 — they were `bodyMedium`, so the paid chapters looked less important than the free ones). The dead `silhouette` prop was deleted from 7 call sites (D47) and `CompareCard` finally became the left-aligned row-card its own comment claimed (D49). Gates green, 125 tests. |
| 2026-07-25 | U4.T2 | **The reveal's share affordance stops being a second copy of its own stamp (CO-3).** `SealFab` was a claret `Logomark` seal identical to the `ReadyStamp` 600px above it — two of the same mark, one of them dead — unlabeled, at a flat `spacing.xl` from the edge (i.e. under the home indicator on an inset device), with `shadow.md` attached to a **transparent** `View`, which casts nothing on iOS and a rectangular halo on Android. It is now a labeled pill: `share` icon + "Share" on a real `surface` fill with a hairline border, `radii.pill`, the 44pt floor, `shadow.md` on the SAME filled view (iOS shadow props + `elevation: 4`), bottom-right at `insets.bottom + spacing.lg`. **Measured in the web export at 390×844, both schemes, after a programmatic scroll past the 240px threshold:** stamps on the page = **1** (was 2 in the source at HEAD), pill box 104×44 with a 16pt gap to both the right and bottom viewport edges, `role="button"`, label "Share", icon present, background resolved opaque (`rgb(255,255,255)` light / `rgb(30,32,39)` dark), 1px border, 999px radius, boxShadow rendered — and **absent before the scroll**, so the affordance is still earned. The safe-area *term* is code-level (a web viewport reports 0 insets); the old flat 24 could not clear a home indicator at all. Gates green, 119 tests. |
| 2026-07-25 | U4.T1 | **The reveal's tail stops burying the trust story (CO-2, SH-10).** `TrustFooter` sat literally BETWEEN the second-hand card and the other-kind card — the promise about the reader's photo sandwiched between two ads. Order is now sections → Compare → Go deeper → ONE "Continue" card → trust → disclaimer, **measured by document position**: compare@1279 < deeper@1758 < keep@2037 < trust@2268 < disclaimer@2394. The two offer cards consolidate into one dismissible `ContinueCard` whose dismissal **persists** and whose rows hide once that reading exists (SH-10 — "Add your other hand" previously showed on every palm reveal forever); already-owned comes from the reader's own shelf (D45). The survey moved BELOW section 1 (it asked "does this match what you remember?" above the reading) with kind-aware copy, and its buttons now wrap — `md` alone was still 33pt over the card box (D44). `CompareCard` flattened from the page's only centred `md` island. Gates green, 119 tests. |
