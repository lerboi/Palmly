...
- [x] **B18 — M10: deprecate the un-deduped push enqueue path** *(M10)* — **2026-07-17**
  - **DONE: M10 CONFIRMED (both limbs — the duplicate path AND the audit's secondary note).** Landed
    as `supabase/migrations/20260717000029_m10_drop_raw_enqueue_push.sql` (counter re-read: max was 28).
  - **CONFIRMED:** `enqueue_push` (`0013:4-20`) calls `queue_send` directly — **no dedupe, no cap** —
    and its own header calls it *"the single enqueue entry point the pipeline uses"*, which
    `enqueue_push_deduped` (`0014`) silently superseded without deprecating it. Both live, both
    granted to service_role, so §10 was enforced only by **remembering which function to call**.
  - **Callers checked before contracting: the raw path had ZERO production callers.** The only real
    enqueuer — `worker-compat/index.ts:110` — already used the deduped path; the single reference
    anywhere was a **test**, migrated alongside. So "migrate any caller" was a one-line test change.
  - **DROPPED, not merely revoked (D-26).** Verified live afterwards: the only surviving function is
    `enqueue_push_deduped(...)`.
  - 🎯 **The audit's "secondary note" was the better half of this finding, and the fix was one word.**
    The marketing cap was **check-then-insert**: `0014:56-63` asked *"does a marketing row exist for
    (user, today)?"* and only then inserted — so two concurrent marketing sends both saw nothing, both
    passed, and both inserted (**their dedupe keys differ**, so the `(user, dedupe_key, sent_on)`
    index did not stop them). Two marketing pushes, one day. Same shape as M8: *a check in one
    statement and an act in another is not a gate.*
    **`notification_log_marketing_idx` (`0014:25`) ALREADY had exactly the right columns and
    predicate** — `(user_id, sent_on) WHERE cap_class = 'marketing'` — the author built it to make the
    `exists` lookup fast rather than to **be** the invariant. Replaced it with the **UNIQUE** version,
    deleted the pre-check, and switched to an **untargeted `on conflict do nothing`**, which catches
    *both* indexes at once (dedupe key **and** cap). Semantics are unchanged (both already returned
    null) — they are simply atomic now, with one path instead of two. Safe: `notification_log` holds
    **0 rows / 0 groups would violate** (verified before writing).
  - Verify (observed): `git grep -n "enqueue_push\b" -- supabase` → **0 remaining callers** of the raw
    path (the only hits are prose and the new test that asserts its absence) ✅ · live MCP: surviving
    fns = **`enqueue_push_deduped` only**, cap index = `CREATE UNIQUE INDEX … WHERE (cap_class =
    'marketing')`, old non-unique index **gone** · `notification_dedupe + push_dispatch + queues`
    **19/19** · full **Node 133/133** (`# pass 133 / # fail 0`, 282.1s) · full **Deno 154/154**.
  - Regression tests added (3, +1 migrated): the raw `enqueue_push` **does not exist** in `pg_proc`
    while the gated path keeps its service-role-only posture; the cap index is **UNIQUE + partial**
    and the function's **pre-check is gone** in favour of `on conflict do nothing`; and the behaviour
    that matters — **a second marketing push the same day is refused even with a different dedupe
    key**, while exempt (social/pipeline) classes still enqueue freely, leaving exactly one marketing
    row per day.
  - **UTC-day window: NOT changed — accepted with the residual stated (D-27).**
