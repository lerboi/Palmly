...
| Field | Value |
|---|---|
| **Current phase** | **P5 — AI pipeline (device-free). P5.T4 `[x]` (KB v1). T1-T3 `[~]` (live legs gated on H4c+images).** P3.T5/T6 `[~]` (human-gated legs). P4 capture device-blocked. |
| **Next task** | **P5.T6** `worker-narrative` (Flash-Lite narrative grounded on features+KB only, no image; sections schema, `depth_level`, version stamps — Backend §6.3). Device-free & **mock-testable now**: uses schema-valid mock feature_sets + the real KB v1 (P5.T4); `generateContent` is functional on the current key (the H4c gap is caching + image-data-training, neither of which the text-only narrative pass needs). First run the ∥ human **P5.T5** (KB authenticity review, **H6b**) — check `kb/REVIEW.md`; absent → skip past it (∥) to T6. P5.T1/T2/T3 `[~]` live legs still pending **H4c**+real palm images. |
...
| **Last completed task** | **P5.T4 `[x]`** — KB v1: `kb/v1/{palmistry,physiognomy}.json` = **141 chunks**, one per `(feature, enum-value)`; `kb/audit.mjs` coverage **141/141** + banned-claims clean (`P5T4_OK`); `kb.test.mjs` proves load + keyed lookup vs staging. **Backend suite now 34/34.** |
| **Notes for next run** | **P5.T4 done (2026-07-13): KB v1 built** — `kb/v1/{palmistry,physiognomy}.json` (141 chunks, atomic `(feature,enum-value)` keys, classical 手相/面相, no-health), `kb/audit.mjs` (schema-derived coverage 141/141 + banned-claims), `kb/load.mjs` (byte-stable rows), `kb.test.mjs` (load+keyed-lookup vs staging). **Backend suite 34/34.** H4c re-checked at start of this run → still free-tier (`FreeTier limit=0` on explicit cache); P5.T1/T2/T3 live legs stay parked. <br>**NEXT (device-free): P5.T6 `worker-narrative`** (Backend §6.3). It's text-only (features+KB, no image), so **build + test it now** without H4c: feed **schema-valid MOCK feature_sets** (author 5 under e.g. `eval/samples/` — palm_valid.json already exists) + the real KB v1 via deterministic keyed lookup; `generateContent` is functional on the current key for the actual narrative call (only caching + image-data-training are H4c-gated, and the narrative pass uses neither). Author `schemas/reading_sections.v1.json` + `prompts/narrative/v1/`, build `_shared` narrative lib + `worker-narrative` Deno fn (dequeue `narrative_jobs` → generate → validate → store `readings` with `model_id/prompt_version/kb_version/extractor_version` stamps). Verify: 5 mock sets → sections-schema-valid; regen from identical features → identical tagged claims; no-medical-claims audit (reuse `kb/audit.mjs` banned lexicon). First check ∥ **P5.T5** (`kb/REVIEW.md`, human **H6b**) — absent → skip past (∥). <br>**Backend dev/test model:** migrations in `supabase/migrations/`; `cd supabase/tests && node --test` (34/34, rolled-back vs staging); Deno for edge fns. Persist to staging only when needed: `CONFIRM=1 node supabase/tests/scripts/apply.mjs` (schema), `CONFIRM=1 node supabase/tests/scripts/load-kb.mjs` (KB). <br>**All app/device work (P1.T4, P1.T6, P2, P4, P6) stays paused on Human-tasks.md** (device H1, PostHog/Sentry H2). **Human quick wins that unblock a lot:** H4c (paid Gemini → unparks P5.T1/2/3 live), H5 (anon toggle), H4b (deploy token), H1 (device). |
