...
| **Next task** | **P5.T2** (`worker-scan` extraction). Build the Deno worker + logic tests with MOCKED Gemini (device-free); the LIVE "5 palm images → 5/5 schema-valid" verify needs test palm images (try Gemini image-gen, else human) + a paid key (**H4c**). P5.T1 `[~]` caching pending H4c. |
