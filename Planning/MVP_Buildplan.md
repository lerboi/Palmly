...
| Field | Value |
|---|---|
| **Current phase** | **P3 — backend foundation (Docker-free), reordered ahead of device-blocked P2** (user-authorized) |
| **Next task** | **P3.T6** — anonymous-first auth in the app (`signInAnonymously`, profile row, Turnstile). Turnstile leg needs H5; do the rest. (P3.T5 `[~]` CI-deploy pending H4b.) |
...
| **Last completed task** | P3.T4 (queues + cron + telemetry; suite 25/25) |
| **Notes for next run** | **Loop reordered to P3 (backend) per user; building Docker-free.** Path: P3.T2 → T3 (storage) → T4 (queues) → T5 (edge fns) → T6 (anon auth; needs Turnstile H5 for the CAPTCHA leg — do the rest, park that leg) → P5 (AI pipeline; needs `GEMINI_API_KEY` in Vault H4 for live worker verify — build + unit-test what's possible). **Backend dev/test model:** write migration files in `supabase/migrations/`; test via `cd supabase/tests && node --test` (applies + asserts RLS against staging in rolled-back txns — see `docs/SETUP.md#backend`). Persistently deploy to staging only when a phase needs it (`CONFIRM=1 node scripts/apply.mjs`). **App-side (P1.T4/T5/T6, P2) remains paused on human items in Human-tasks.md** — resume the instant a device + PostHog/Sentry keys land. Android dev build `e081b5db` may have finished: https://expo.dev/accounts/lehehroi/projects/palmly/builds/e081b5db-97c2-4444-a5fa-c88a232a6ee7. |
