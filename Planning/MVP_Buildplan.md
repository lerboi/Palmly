...
| **Next task** | **P5.T1** (extraction prompts + schemas + Gemini cached content) — device-free, testable with the local `GEMINI_API_KEY` + Deno + transactional DB. (P4 capture UX skipped: needs a camera/device → Human-tasks H1. P3.G gate pending H4b+H5+H1.) |
