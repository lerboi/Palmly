...
| **Next task** | **P5.T3** (consistency layer: subject-identity matching via `geometryDistance` vs `subject_profiles`, canonical-reuse fast path, 2-vote + tie-break extraction). Device-free, testable transactionally + Deno. P5.T1/T2 `[~]` live-Gemini legs pending **H4c** paid key + test images. |
...
| **Last completed task** | P3.T6 backend + app code (`[~]` runtime gated); P3.T1-T4 `[x]`; suite 26/26 |
