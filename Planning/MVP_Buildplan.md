...
| Field | Value |
|---|---|
| **Current phase** | **P3 — backend (Docker-free) nearly complete: T1-T4 `[x]`, T5/T6 `[~]` (human-gated legs).** Next continuable: **P5 (AI pipeline, device-free)** — P4 capture is device-blocked. |
| **Next task** | **P5.T4** (KB v1: author `kb/v1/` palmistry+physiognomy entries keyed by `feature_key`, load into `kb_chunks`, deterministic keyed lookup; coverage script every enum combo → ≥1 chunk; banned-claims grep audit). Device-free, no Gemini. Then P5.T6 `worker-narrative` (mock-testable; live pending **H4c**). P5.T1/T2/T3 `[~]` live legs pending **H4c**+images. |
...
| **Last run (date, by whom)** | 2026-07-12, agent |
| **Last completed task** | P5.T3 consistency (`[~]` live-legs gated); P5.T1/T2 `[~]`; P3.T1-T4 `[x]`. Backend suite 30/30, Deno 27/27 |
| **Notes for next run** | **P3 backend built Docker-free (transactional tests vs staging).** Done `[x]`: P3.T1-T4 (harness, schema+RLS 26 proofs, storage, queues). `[~]` (code done, human-gated runtime): **P3.T5** edge-fn skeleton+`_shared/` (deploy needs H4b token), **P3.T6** anon-auth trigger+app (needs H5 anon-enable + H1 device). **Backend dev/test model:** migration files in `supabase/migrations/`; `cd supabase/tests && node --test` (26/26, rolled-back vs staging); Deno for edge fns (`_shared` + `hello`, 10/10, live-curl verified). Persist to staging only when needed: `CONFIRM=1 node supabase/tests/scripts/apply.mjs`. <br>**NEXT continuable (device-free): P5 (AI pipeline).** P4 (capture UX) is camera/**device-blocked → skip** (Human-tasks H1). Build P5 workers (`worker-scan`/`worker-narrative`) as Deno edge fns; test locally with the **local `GEMINI_API_KEY`** (in `.env.staging`) + transactional DB — the Vault key (H4) is only for the *deployed* function. Extraction schema/prompts under `prompts/`, `schemas/`; KB under `kb/`. <br>**All app/device work (P1.T4 device-install, P1.T6 analytics, P2 spike, P4, P6) stays paused on Human-tasks.md** (device H1, PostHog/Sentry H2). **Human quick wins that unblock a lot:** H5 (1-click anon toggle), H4b (deploy token), H1 (device). |
